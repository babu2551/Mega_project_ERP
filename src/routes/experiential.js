...
router.post("/submit", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to save experiential entry" });
  }
});
...
router.get("/entries", authMiddleware, async (req, res) => {
  try {
    if (req.user && req.user.role === "admin") {
      const docs = await Experiential
        .find({})
        .lean()
        .exec();
      return res
        .json(docs.map((d) => ({ id: d._id.toString(), createdAt: d.createdAt, ...d })));
    }
    const docs = await Experiential
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch experiential entries" });
  }
});
...
router.get("/entries/:id", authMiddleware, async (req, res) => {
  try {
    ...
    return res
      .json({ id: doc._id.toString(), createdAt: doc.createdAt, ...doc });
  } catch (err) {
    console.error("Experiential read error", err);
    res
      .status(500)
      .json({ error: "Failed to read experiential entry" });
  }
});
